...
this.nggh_mod_charm_captive_spell <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip() {
		...
		if (!::Nggh_MagicConcept.Mod.ModSettings.getSetting("arena_charm").getValue()
			&& ::Tactical.isActive()
			&& ("StrategicProperties" in ::Tactical.State.m)
			&& ::Tactical.State.m.StrategicProperties != null
			&& ::Tactical.State.m.StrategicProperties.IsArenaMode)
		{
			ret.push({
				id = 99,
				type = "text",
		...
		local flag = ::Tactical.isActive()
			? ::Tactical.Entities.getFlags().getAsInt("CharmedCount")
			: 0;
	...
	function onVerifyTarget(_originTile, _targetTile) {
		...
		if (charm != null) {
			...
			if (charm.m.Master.getContainer() != null
				&& charm.m.Master.getContainer().getActor().getID() != this.getContainer().getActor().getID())
			{
				return false;
			}
		}
	...
	function isViableTarget(_user, _target) {
		...
		if (_target.getType() == ::Const.EntityType.Hexe
			|| _target.getType() == ::Const.EntityType.LegendHexeLeader)
		{
			return false;
		}
	...
	function incrementCharmFlag(_targetEntity) {
		...
		if (isLindwurm) {
			if (::MSU.isKindOf(_targetEntity, "lindwurm_tail")
				|| ::MSU.isKindOf(_targetEntity, "legend_stollwurm_tail"))
			{
	...
	function onCharm(_targetEntity, _victim, _isAFuckingLindwurm = false) {
		// Legends doesn't count tails as enemies in getHostilesNum(), unlike Vanilla.
		local tail = _isAFuckingLindwurm ? _targetEntity.m.Tail : null;
		local isTailCounted = tail != null && !tail.isNull() && !tail.getFlags().has("tail");
		local isFinish = ::Tactical.Entities.getHostilesNum() <= (isTailCounted ? 2 : 1);
	...
	function getHitchance(_targetEntity) {
		...
		bonus += this.getContainer().hasSkill("perk.bdsm_bondage")
			&& _targetEntity.getCurrentProperties().IsRooted ? 7 : 0;
		bonus += this.getContainer().hasSkill("perk.bdsm_whip_love")
			? ::Math.min(20, _targetEntity.getFlags().getAsInt("whipped") * 4)
			: 0;
		bonus += this.getBonus();
		local defenderProperties = _targetEntity.getSkills().buildPropertiesForDefense(_user, this);
		local resist = (defenderProperties.getBravery() + defenderProperties.MoraleCheckBravery[1]) * defenderProperties.MoraleCheckBraveryMult[1] * (_targetEntity.getSkills().hasSkill("racial.champion")
			? this.m.ChampionMult
			: 1.0);
		...
		for (local i = 0; i != 6; ++i) {
			if (!targetTile.hasNextTile(i)) {
				...
				if (tile.IsOccupiedByActor
					&& tile.getEntity().getMoraleState() != ::Const.MoraleState.Fleeing)
				{
	...
	function getHitFactors(_targetTile) {
		...
		local defenderProperties = _targetEntity.getSkills().buildPropertiesForDefense(_user, this);
		local resist = ::Math.floor((defenderProperties.getBravery() + defenderProperties.MoraleCheckBravery[1]) * defenderProperties.MoraleCheckBraveryMult[1] * (_targetEntity.getSkills().hasSkill("racial.champion")
			? this.m.ChampionMult
			: 1.0));
		...
		if (defenderProperties.MoraleCheckBraveryMult[::Const.MoraleCheckType.MentalAttack] >= 1000.0
			|| (resist >= 500 && isFullHP))
		{
			ret.push({
				icon = "ui/icons/cancel.png",
		...
		if (targetEntity.getType() == ::Const.EntityType.Hexe
			|| targetEntity.getType() == ::Const.EntityType.LegendHexeLeader)
		{
			ret.push({
				icon = "ui/icons/cancel.png",
		...
		if (this.getContainer().hasSkill("perk.bdsm_bondage")
	...
	function checkRequirement(_targetEntity, _isForToolTips = false) {
		...
		if (_targetEntity.getSkills().hasSkill("racial.champion")
			&& requirements.find("NggHCharmSpec") == null)
		{
	...
	function onCombatFinished() {
		...
		local isVictory = ::Tactical.Entities.getCombatResult() == ::Const.Tactical.CombatResult.EnemyDestroyed
